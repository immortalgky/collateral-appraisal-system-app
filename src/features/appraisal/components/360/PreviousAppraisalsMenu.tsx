    ...
    <div className="relative">
      ...
      {isOpen && (
        <>
          ...
          <div className="absolute left-0 top-full mt-1 w-80 bg-white border border-gray-200 rounded-lg shadow-lg z-40 max-h-80 overflow-y-auto">
            {items.map(item => (
              <button
                key={item.appraisalId}
                type="button"
                onClick={() => handleSelect(item.appraisalId)}
                className="flex w-full flex-col gap-1 px-3 py-2.5 text-left hover:bg-gray-50 transition-colors border-b border-gray-100 last:border-b-0"
              >
