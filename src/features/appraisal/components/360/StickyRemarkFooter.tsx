...
const StickyRemarkFooter = ({ taskId, taskDraft, canEdit }: StickyRemarkFooterProps) => {
  ...
  // Seed local draft ONCE from the fetched value so a post-save refetch doesn't
  // clobber an in-progress edit.
  const seededRef = useRef(false);
  const [draft, setDraft] = useState('');
  useEffect(() => {
    if (seededRef.current || taskDraft === undefined) return;
  ...
  return (
    <div className="pointer-events-none absolute inset-x-6 bottom-6 z-30 flex flex-col items-end gap-3">
      {/* Expanded comment card */}
      {open && (
        <div className="pointer-events-auto w-full overflow-hidden rounded-2xl border border-gray-200 bg-white shadow-2xl ring-1 ring-black/5 transition">
          {/* Header — colorized */}
      ...
      <button
        ...
        className={clsx(
          'pointer-events-auto relative flex h-14 w-14 items-center justify-center rounded-full text-white shadow-lg transition-all duration-200 hover:scale-105 active:scale-95',
          open
