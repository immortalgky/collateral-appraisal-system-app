  ...
  return (
    <div className="space-y-4">
      ...
      <div className="space-y-1 px-3 py-2 rounded-lg bg-gray-50 border border-gray-200">
        ...
        <DlRow
          label="Land Growth Rate Type"
          value={a.landGrowthRateType}
          isText
        />
            ...
            <table className="w-full text-xs whitespace-nowrap">
              <thead>
                <tr className="bg-gray-100">
                  ...
                  <th className="px-2 py-1.5 text-right font-medium text-gray-500">Bldg After Depr</th>
                  <th className="px-2 py-1.5 text-right font-medium text-gray-500">Total L+B</th>
                  <th className="px-2 py-1.5 text-right font-medium text-gray-500">Rental Income</th>
                  <th className="px-2 py-1.5 text-right font-medium text-gray-500">PV Factor</th>
                  <th className="px-2 py-1.5 text-right font-medium text-gray-500">Net Curr. Rental</th>
                </tr>
              </thead>
              <tbody className="divide-y divide-gray-50">
                {a.calculationDetails.map((row: LeaseholdCalculationDetail) => (
                  <tr key={row.year}>
                    ...
                    <td className="px-2 py-1 text-right text-gray-600">{formatNumber(row.landValue, 2)}</td>
                    <td className="px-2 py-1 text-right text-gray-600">{formatNumber(row.landGrowthPercent, 2)}%</td>
                    <td className="px-2 py-1 text-right text-gray-600">{formatNumber(row.buildingValue, 2)}</td>
                    <td className="px-2 py-1 text-right text-gray-600">{formatNumber(row.depreciationAmount, 2)}</td>
                    <td className="px-2 py-1 text-right text-gray-600">{formatNumber(row.depreciationPercent, 2)}%</td>
                    <td className="px-2 py-1 text-right text-gray-600">{formatNumber(row.buildingAfterDepreciation, 2)}</td>
                    <td className="px-2 py-1 text-right text-gray-600">{formatNumber(row.totalLandAndBuilding, 2)}</td>
                    <td className="px-2 py-1 text-right text-gray-600">{formatNumber(row.rentalIncome, 2)}</td>
                    <td className="px-2 py-1 text-right text-gray-600">{formatNumber(row.pvFactor, 6)}</td>
                    <td className="px-2 py-1 text-right font-medium text-gray-800">{formatNumber(row.netCurrentRentalIncome, 2)}</td>
                  </tr>
                ))}
              </tbody>
            </table>
