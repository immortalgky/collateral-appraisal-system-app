...
export const mapConstructionInspectionResponseToForm = (ci: any) => ({
  ...
  constructionSubItems: (ci?.workDetails ?? []).map((wd: any) => ({
    ...
    currentProgressPct: wd.currentProgressPct ?? wd.previousProgressPct ?? 0,
  })),
  constructionSummary: {
    ...
    summaryCurrentProgressPct: ci?.summaryCurrentProgressPct ?? ci?.summaryPreviousProgressPct ?? 0,
...
export const mapMachineryPropertyResponseToForm = (
  ...
  // Must agree with createMachineryFormDefault field for field: react-hook-form takes whatever
  // this returns as the reset() baseline, and a text field seeded null never compares equal to
  // the '' its input emits — which is what left the form permanently "unsaved".
  return {
    // One name field on the form now, but two columns behind it: a machine entered through the
    ...
    registrationStatus: response.registrationStatus ?? false,
    invoiceNumber: response.invoiceNumber ?? '',
    isPriceCertified: response.isPriceCertified ?? true,
    ...
    machineDimensions: response.machineDimensions ?? '',
    energyUse: response.energyUse ?? '',
    location: response.location ?? '',
    conditionUse: response.conditionUse ?? '',
    machineCondition: response.machineCondition ?? '',
    machineAge: response.machineAge ?? null,
    machineEfficiency: response.machineEfficiency ?? '',
    machineTechnology: response.machineTechnology ?? '',
    usagePurpose: response.usagePurpose ?? '',
    machineParts: response.machineParts ?? '',
    replacementValue: response.replacementValue ?? null,
    conditionValue: response.conditionValue ?? null,
    other: response.other ?? '',
    remark: response.remark ?? '',
    appraiserOpinion: response.appraiserOpinion ?? '',
  };
};
