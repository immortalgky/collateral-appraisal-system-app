...
const LandTitleTable = ({
  ...
  // Every field gets a column. The shared table drops conditional ones and squeezes their
  // value onto the end of whatever field their `showWhen` points at — which is where the
  // "Title deed - -" in the Title Type column came from. That merge only ever showed the
  // first dependant, so Rawang, Land Number, Survey Number, Sheet Number and both Aerial
  // Photo columns had nowhere to appear at all. A correction screen has to show every field
  // it can correct, so nothing is folded away here.
  ...
  const computeOffsets = useCallback(() => {
    ...
    // Each offset is derived from the *previous pinned offset* plus that column's width, then
    // floored — not from a floored running total of true widths. The difference is the white
    // seam: with columns of 30.4 and 71.6, flooring the true total pins the third column at
    // 102 while the second ends at 101.6, leaving 0.4px of table showing through. Chaining the
    // floor pins it at 101 instead, so sticky cells always overlap by a fraction rather than
    // parting. getBoundingClientRect because offsetWidth is already rounded.
    let left = 0;
  ...
  useEffect(() => {
    computeOffsets();
    // Column widths change with the container, the density setting and font loading, none of
    // which are covered by a row-count dependency — a stale offset is what produces the seam.
  ...
  return (
    ...
    // `flex flex-col`, not `grid`. As a grid the scroll box and the "Add item" row are
    // grid items whose column is sized against the table's `min-w-max` content, which
    // pushed the button out beside the table instead of under it. A column flex box has
    // no such interaction: each child is a full-width block, in order.
    <div data-field={name} className="cas-repeater flex w-full min-w-0 flex-col">
      {isEmpty ? (
        ...
        <>
          <div className="max-h-60 overflow-auto rounded border border-gray-200 min-w-0">
            <table ref={tableRef} className="cas-sticky-table table table-zebra table-xs min-w-max">
              <thead>
                <tr>
                  {showRowNumber && (
                    <th
                      className="bg-primary-700 text-white text-xs font-medium py-2 px-3 text-left sticky z-10"
          ...
          {!readOnly && (
            <div className="w-full min-w-0 border-t border-gray-100">
              <button
                type="button"
                onClick={() => setModalState({ type: 'add' })}
                className="flex w-full items-center justify-center gap-2 rounded-b bg-gray-50 py-2 text-xs font-medium text-primary-600 transition-colors hover:bg-primary-50"
              >
