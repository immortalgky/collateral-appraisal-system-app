...
import {
  PropertyNameTriggerIcon,
  type PropertyType,
} from '@/features/appraisal/components/PropertyNameTriggerIcon';
import { useFormContext } from 'react-hook-form';
import { useTranslation } from 'react-i18next';

interface BuildingDetailFormProps {
  /**
   * `prefix` is gone with the Building Detail section: it existed only to namespace the
   * depreciation field array, and nothing else in this form is nested.
   */
...
const BuildingDetailForm = ({ propertyType = 'B' }: BuildingDetailFormProps) => {
  ...
  return (
    <div className="cas-section-grid grid grid-cols-1 xl:grid-cols-5 gap-6">
      ...
      {/* The create screen's "Building Detail" section — total area plus the RCN /
          depreciation table — is absent here on purpose. Both are valuation inputs and the
          correction API carries neither: BuildingCorrection has no TotalBuildingArea, and
          depreciation rows were never DTO members at all, so edits there saved into nothing.
          Guarded on the backend by CorrectionDto_DoesNotExposeArea. */}
