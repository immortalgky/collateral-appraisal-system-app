...
import { useGetCondoTowers } from '../../api/condoTower';
...
export default function TowerListingTab() {
  ...
  const { data: towersData, isLoading, isError } = useGetCondoTowers(appraisalId ?? '');
  const towers = Array.isArray(towersData) ? towersData : Array.isArray(towersData?.towers) ? towersData.towers : [];
