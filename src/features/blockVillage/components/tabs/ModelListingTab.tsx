...
import { useGetVillageModels } from '../../api/villageModel';
    ...
    return (
      ...
      >
        <div className="w-16 h-16 rounded-lg bg-gray-100 flex items-center justify-center shrink-0">
          <Icon name="house" style="solid" className="text-gray-400 w-7 h-7" />
        </div>
...
export default function ModelListingTab() {
  ...
  const { data: modelsData, isLoading, isError } = useGetVillageModels(appraisalId ?? '');
  const models = Array.isArray(modelsData) ? modelsData : Array.isArray(modelsData?.models) ? modelsData.models : [];

  const handleModelClick = (modelId: string) => {
    navigate(`${basePath}/block-village/model/${modelId}`);
  };

  const handleAddModel = () => {
    navigate(`${basePath}/block-village/model/new`);
  };
    ...
    <div className="flex flex-col h-full min-h-0">
      ...
      {models.length === 0 ? (
        <div className="flex flex-col items-center justify-center py-16 text-gray-400 bg-gray-50 rounded-xl border-2 border-dashed border-gray-200">
          <Icon name="house" className="text-4xl mb-3" />
