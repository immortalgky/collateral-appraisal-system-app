...
import {
  usePendingFollowups,
  useMonitoringFollowupsSummary,
  useMonitoringFollowupsGrouped,
...
interface PendingFollowupSectionProps {
  onCountChange?: (count: number) => void;
}

function PendingFollowupSection({ onCountChange }: PendingFollowupSectionProps) {
  const { t } = useTranslation('monitoring');
  ...
  const { data, isLoading, isError, error } = usePendingFollowups(filter);

  const { data: summary, isLoading: summaryLoading } = useMonitoringFollowupsSummary(baseFilter);

  const { data: groupedData, isLoading: groupedLoading } = useMonitoringFollowupsGrouped(
    groupBy,
    baseFilter,
  );
  ...
  const taskTypeOptions = useMemo(
    () => [
      ...
      { value: 'ProvideAdditionalDocuments', label: 'Provide Additional Documents' },
    ],
    [],
  );
      ...
      <div className="shrink-0 mb-3 flex flex-wrap items-center gap-2">
        ...
        <div className="flex-1 min-w-[200px] max-w-xs">
          <Input
            ...
            placeholder={t('pendingFollowup.search')}
...
export default PendingFollowupSection;
