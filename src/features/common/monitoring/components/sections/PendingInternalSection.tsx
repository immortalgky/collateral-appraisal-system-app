...
interface PendingInternalSectionProps {
...
function PendingInternalSection({ onCountChange }: PendingInternalSectionProps) {
  ...
  const filter: PendingInternalFilter = {
    ...baseFilter,
  ...
  const { data, isLoading, isError, error } = usePendingInternal(filter);

  const { data: summary, isLoading: summaryLoading } = useMonitoringInternalSummary(baseFilter);

  const { data: groupedData, isLoading: groupedLoading } = useMonitoringInternalGrouped(
    groupBy,
    baseFilter,
  );
      ...
      <div className="shrink-0 mb-3 flex flex-wrap items-center gap-2">
        ...
        <div className="flex-1 min-w-[200px] max-w-xs">
          <Input
            ...
            placeholder={t('pendingInternal.search')}
...
export default PendingInternalSection;
