    ...
    <dialog className="modal modal-open">
      <div className="modal-box bg-white rounded-2xl shadow-xl max-w-md">
        <div className="flex items-center justify-between mb-6">
          <h3 className="font-semibold text-xl text-gray-800">{t('addWidget.title')}</h3>
          <button
            type="button"
            onClick={onClose}
        ...
        {hiddenWidgets.length > 0 ? (
          <div className="space-y-6">
                      ...
                      >
                        ...
                        <div className="flex-1">
                          ...
                          <p className="text-sm text-gray-500">
                            {t(`widgetConfigs.${configKey}.description`)}
                          </p>
                        </div>
            ...
            {smallWidgets.length > 0 && (
              <div>
                <h4 className="text-xs font-semibold text-gray-400 uppercase tracking-wider mb-3">
                  {t('addWidget.smallWidgets')}
                </h4>
                <div className="grid gap-3">
                  {smallWidgets.map(widget => {
                    const configKey = WIDGET_TYPE_TO_CONFIG_KEY[widget.type];
                    return (
                      <div key={widget.type} className="border border-gray-100 rounded-xl p-4">
          ...
          <div className="text-center py-10">
            ...
            <p className="text-gray-600 font-medium">{t('addWidget.allVisible')}</p>
            <p className="text-sm text-gray-400 mt-1">{t('addWidget.allVisibleHint')}</p>
          </div>
        )}
