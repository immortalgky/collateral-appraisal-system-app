...
export default function NotificationList() {
  const { notifications, toggleRead } = useNotificationStore();
    ...
    <div className="divide-y divide-gray-100 bg-white rounded-xl ring-1 ring-gray-200">
      {notifications.map(notification => {
        ...
        return (
          ...
          >
            <div className={clsx('mt-0.5 flex-shrink-0 w-10 h-10 rounded-xl flex items-center justify-center', config.color)}>
              <Icon name={config.icon} style={config.iconStyle} className="size-5" />
            </div>
            <div className="flex-1 min-w-0">
              <div className="flex items-center gap-2">
                <p className={clsx('text-sm', !notification.isRead ? 'font-semibold text-gray-900' : 'font-medium text-gray-700')}>
                  {notification.title}
                </p>
                {!notification.isRead && (
