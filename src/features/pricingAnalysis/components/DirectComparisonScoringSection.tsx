...
import { deriveSecondRevisionVisibility } from '../domain/deriveSecondRevisionVisibility';
...
export const DirectComparisonScoringSection = ({
  ...
  // via marketSubLabel.ts; needed for the market header to legitimately reach 36px.
   ...
   * as WQSScoringSection.tsx / SaleAdjustmentGridScoringSection.tsx). Verified safe the
   * same way: rows are keyed by factorCode/factorId, market cells carry their own
   * marketId in the submit payload, and displaySequence is the row's array index — none
   * of that depends on which columns are frozen. Market columns are not reordered
  ...
  // DirectComparisonSecondRevision renders into this same <table> — custom properties
  ...
  // factor, read-only) 110px, then ระดับ (level) 82px — same numbers as
  // SaleAdjustmentGridScoringSection.tsx. HANDOFF 14d / mock:1503: the editable
  // percentage left this grid and now lives only in the ADJUSTMENT & SUMMARY band below.
  ...
  // and 150 is its ceiling for the same reason: without one, a long value stretched this
  // one column to 383px under table-layout: auto (measured in the sibling SAG grid),
  // which broke market-to-market alignment and — through the shared <colgroup> —
  // squeezed that market's Amount cell in the band below. `overflow-hidden` alone never
  // fired because the cell simply grew. Same 150px ceiling as collateralColumnBody,
  // which already truncates this exact text; FactorValueDisplay wraps its output in
  // `truncate` with a title, so the full value stays available on hover. The level
  // column keeps no ceiling — it's the one that should absorb the pair's surplus width.
        ...
        >
          ...
          <thead className="bg-gray-50">
            <tr className="border-b border-gray-200">
              ...
              >
                {t('directComparison.scoringTable.factorsHeader')}
              </th>
              <th
                rowSpan={comparativeSurveys.length > 0 ? 2 : 1}
                className={clsx(
              ...
              >
                {t('directComparison.scoringTable.collateralHeader')}
              </th>
              ...
              {comparativeSurveys.map((survey: MarketComparableDetailType) => (
                <Fragment key={survey.id}>
                  <th className="bg-gray-50 font-medium text-left px-[8px] py-0 h-[26px] w-[110px] min-w-[110px] max-w-[150px] border-b border-b-gray-300 border-r border-r-[#eef2f2] sticky top-[36px] z-23 whitespace-nowrap">
                    {t('directComparison.scoringTable.valueHeader')}
                  </th>
                  <th className="bg-gray-50 font-medium text-left px-[8px] py-0 h-[26px] w-[82px] min-w-[82px] border-b border-b-gray-300 border-r border-r-[#eef2f2] sticky top-[36px] z-23 whitespace-nowrap">
                    {t('directComparison.scoringTable.levelHeader')}
                  </th>
                </Fragment>
              ))}
            </tr>
          ...
          <tbody className="divide-y divide-gray-100">
            {bandRow(
              t('directComparison.rows.comparativeFactorsBand'),
              isFactorBandOpen,
            ...
            {isFactorBandOpen && (
              <>
                {/* qualitative section */}
                {qualitativeFactorFields.map((fields, rowIndex: number) => {
                  const selected =
                  ...
                  return (
                    <tr key={fields.id} className="group/row">
                        ...
                        <div className="flex items-center gap-1">
                              ...
                              {isTemplateFactor ? (
                                ...
                                <RHFInputCell
                                  ...
                                  onSelectChange={value => {
                                    ...
                                    setValue(
                                      `directComparisonQualitatives.${rowIndex}.factorId`,
                                      fid,
                                    );
                                    setValue(
                          ...
                          {!isTemplateFactor && (
                            <button
                              ...
                              title={t('directComparison.rows.deleteTitle')}
                            >
                      ...
                      {/* ค่าปัจจัย / ระดับ split (HANDOFF 14d, mock:1503/1511) — the grid
                          ...
                          via buildDirectComparisonAdjustmentFactorDefaultPercentRules,
                          which only fires while the percentage is still empty. */}
                        ...
                        <Fragment key={survey.id}>
                          ...
                          <td className={levelColumnBody}>
                            ...
                            <RHFInputCell
                              ...
                              options={[
                                {
                                  label: t('directComparison.rows.levelEqual'),
                                  value: 'E',
                                  colorClass: 'text-gray-600',
                                },
                                {
                                  label: t('directComparison.rows.levelInferior'),
                                  value: 'I',
                                  colorClass: 'text-red-600',
                                },
                                {
                                  label: t('directComparison.rows.levelBetter'),
                                  value: 'B',
                ...
                <tr>
                  ...
                  <td className={clsx('bg-white', leftColumnBody)}>
                    {!isReadOnly && (
                      ...
                      >
                        {t('directComparison.rows.addMoreFactors')}
                      </button>
                    )}
                  </td>
            ...
            {bandRow(t('directComparison.rows.offeringPriceBand'), isPriceBandOpen, () =>
            ...
            {isPriceBandOpen && (
              <>
                <tr>
                  <td className={clsx('bg-white', leftColumnBody, bgGradient)}>
                    <span>{t('directComparison.rows.offeringPrice')}</span>
                  </td>
                  <td className={clsx('bg-white', collateralColumnBody)}></td>
                ...
                <tr>
                  <td className={clsx('bg-white', leftColumnBody, bgGradient)}>
                    <div className={'flex flex-rows justify-between items-center'}>
                      <span>{t('directComparison.rows.adjustedOfferingPriceLabel')}</span>
                      <span>{t('directComparison.rows.percentUnit')}</span>
                    </div>
                  </td>
                  <td className={clsx('bg-white', collateralColumnBody)}></td>
                  {comparativeSurveys.map((survey: MarketComparableDetailType, columnIndex) => {
                ...
                <tr>
                  <td className={clsx('bg-white', leftColumnBody, bgGradient)}>
                    <div className={'flex flex-rows justify-between items-center'}>
                      <span>{t('directComparison.rows.adjustedOfferingPriceLabel')}</span>
                      <span>{t('directComparison.rows.amountUnit')}</span>
                    </div>
                  </td>
                  <td className={clsx('bg-white', collateralColumnBody)}></td>
                  {comparativeSurveys.map((survey: MarketComparableDetailType, columnIndex) => {
                ...
                <tr>
                  <td className={clsx('bg-white', leftColumnBody, bgGradient)}>
                    <span>{t('directComparison.rows.sellingPrice')}</span>
                  </td>
                  <td className={clsx('bg-white', collateralColumnBody)}></td>
                  {comparativeSurveys.map(
                    (survey: MarketComparableDetailType, columnIndex: number) => {
                ...
                <tr>
                  <td className={clsx('bg-white', leftColumnBody, bgGradient)}>
                    {t('directComparison.rows.numberOfYears')}
                  </td>
                  <td className={clsx('bg-white', collateralColumnBody)}></td>
                  {comparativeSurveys.map((survey: MarketComparableDetailType, columnIndex) => {
                    const hasOfferPrice = !!survey.offerPrice;
                ...
                <tr>
                  <td className={clsx('bg-white', leftColumnBody, bgGradient)}>
                    <div className={'flex flex-rows justify-between items-center'}>
                      <span>{t('directComparison.rows.adjustedPeriod')}</span>
                      <span>{t('directComparison.rows.percentUnit')}</span>
                    </div>
                  </td>
                  <td className={clsx('bg-white', collateralColumnBody)}></td>
                ...
                <tr>
                  <td className={clsx('bg-white', leftColumnBody, bgGradient)}>
                    <div className="flex flex-rows justify-between items-center">
                      <span>{t('directComparison.rows.cumulativeAdjustedPeriod')}</span>
                      <span>{t('directComparison.rows.percentUnit')}</span>
                    </div>
                  </td>
                  <td className={clsx('bg-white', collateralColumnBody)}></td>
                ...
                <tr>
                  <td className={clsx('bg-[#f8fafa] font-semibold', leftColumnBody)}>
                    <span>{t('directComparison.rows.adjustedValueLabel')}</span>
                  </td>
            ...
            {(showLandSecondRevision || showBuildingSecondRevision) && (
              <>
                {bandRow(t('directComparison.secondRevision.title'), isSecondRevisionBandOpen, () =>
            ...
            {bandRow(t('directComparison.rows.adjustmentSummaryBand'), isAdjustBandOpen, () =>
              ...
              <>
                ...
                <tr>
                  ...
                  >
                    ...
                    {t('directComparison.rows.factorSubHeader')}
                  </td>
                  ...
                  {comparativeSurveys.map((survey: MarketComparableDetailType) => (
                    <Fragment key={survey.id}>
                      <td className={clsx('bg-gray-100 font-medium text-gray-600', adjustCellBody)}>
                        {t('directComparison.scoringTable.adjustPercentHeader')}
                      </td>
                      <td
                        className={clsx(
                      ...
                      >
                        {t('directComparison.scoringTable.amountHeader')}
                      </td>
                    </Fragment>
                  ))}
                </tr>
                ...
                <tr>
                  ...
                  <td className={clsx('bg-white font-semibold', leftColumnBody, bgGradient)}>
                    <div className="flex flex-row justify-between items-center gap-2">
                      <span>{t('directComparison.rows.totalDifferenceLabel')}</span>
                      <span>{t('directComparison.rows.percentUnit')}</span>
                    </div>
                  </td>
                  <td className={clsx('bg-white', collateralColumnBody)}></td>
                ...
                <tr>
                  <td className={clsx('bg-[#f8fafa] font-semibold', leftColumnBody, bgGradient)}>
                    <div className="flex flex-row justify-between items-center gap-2">
                      <span>{t('directComparison.rows.totalOfAdjustedValue')}</span>
                    </div>
                  </td>
                  <td className={clsx('bg-[#f8fafa]', collateralColumnBody)}></td>
                  {comparativeSurveys.map((survey: MarketComparableDetailType, columnIndex) => {
            ...
            <tr>
              ...
              >
                {t('directComparison.rows.finalValueLabel')}
              </td>
