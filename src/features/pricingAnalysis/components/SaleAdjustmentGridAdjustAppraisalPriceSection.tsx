...
export function SaleAdjustmentGridAdjustAppraisalPriceSection({
  ...
  // table's own Final Value row already renders (SaleAdjustmentGridScoringSection.tsx),
  // which is what mock:1648 asks for: one label per method, shared between that method's
  // table and its summary card, with the source in brackets — ถ่วงน้ำหนัก here, ต่ำสุด for
  // Direct Comparison, การถดถอย for WQS. Hoisted to a const because three branches below
  // render it and one of them concatenates a unit onto it.
  const finalValueLabel = t('saleAdjustmentGrid.rows.finalValueLabel');
  ...
  // re-deciding anything; display-only, see calculateSaleAdjustmentGrid.ts.
  ...
  // row's rounding hint, and "Final Value (weighted)" — the label that names WHICH
          ...
          label={
            <>
              ...
              {/* mock:605 — `.kv .l .en { font-size: 10.5px }`, written as px because the
                  ...
                  WQSAdjustFinalValueSection and DirectComparisonAdjustAppraisalPriceSection. */}
