...
function ComparableTooltip({ active, payload }: any) {
  ...
  return (
    <div style={tooltipStyle} className="bg-white px-3 py-2 shadow-md">
      ...
      <div className="text-gray-600">Adj. Score: {point.adjustmentScore.toFixed(1)}%</div>
    </div>
  );
}
      ...
      <ResponsiveContainer width="100%" height={220}>
        <ScatterChart margin={{ top: 10, right: 20, left: 5, bottom: 10 }}>
          ...
          {comparableData.length > 0 && (
            <Scatter data={comparableData} fill="#f97316" fillOpacity={0.8} r={5}>
              <LabelList dataKey="name" position="top" style={{ fontSize: 8, fill: '#6b7280' }} />
            </Scatter>
          )}

          {/* Subject dot (rendered on top) */}
          {subjectData.length > 0 && (
            <Scatter data={subjectData} fill="#3b82f6" r={8}>
              <LabelList
                dataKey="name"
                position="top"
