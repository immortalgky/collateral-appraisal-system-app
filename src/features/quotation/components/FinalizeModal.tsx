...
const FinalizeModal = ({
  ...
  const onSubmit = (values: FinalizeFormValues) => {
    finalize(
      ...
      {
        onSuccess: () => {
          toast.success(t('toasts.quotationAwarded'));
          handleClose();
        },
        onError: (err: any) => {
          toast.error(err?.apiError?.detail ?? t('toasts.awardFailed'));
        },
      },
    );
  };
  ...
  return (
    <Modal isOpen={isOpen} onClose={handleClose} title={t('finalize.title')} size="md">
      <form onSubmit={handleSubmit(onSubmit)} className="flex flex-col gap-4">
          ...
          <div className="text-sm text-green-700">
            <p
              dangerouslySetInnerHTML={{
                __html: t('finalize.awardBody', { company: `<strong>${companyName}</strong>` }),
              }}
            />
          </div>
        </div>
        ...
        <div>
          <label className="block text-sm font-medium text-gray-700 mb-1">
            {t('fields.notes')}{' '}
            <span className="text-xs text-gray-400">{t('fields.notesOptional')}</span>
          </label>
          <textarea
            {...register('reason')}
            rows={3}
