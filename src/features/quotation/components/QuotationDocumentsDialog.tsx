...
import {
  useGetQuotationDocuments,
  useGenerateQuotationDocument,
  useRemoveQuotationDocument,
} from '../api/quotation';
import { useQuotationDocumentUpload } from '../hooks/useQuotationDocumentUpload';
...
interface QuotationDocumentsDialogProps {
  isOpen: boolean;
  onClose: () => void;
  quotationId: string;
  /**
...
const QuotationDocumentsDialog = ({
  isOpen,
  onClose,
  quotationId,
  selectable = false,
  selectedIds = [],
  onConfirm,
}: QuotationDocumentsDialogProps) => {
  const { t } = useTranslation('quotation');
  const { data: documents = [], isLoading } = useGetQuotationDocuments(
    isOpen ? quotationId : undefined,
  );
  const generate = useGenerateQuotationDocument();
  const removeDocument = useRemoveQuotationDocument();
  const download = useDownloadDocument();
  const viewDocument = useViewDocument();
  const { uploading, uploadAndLink } = useQuotationDocumentUpload(quotationId);
  ...
  const handleDownload = (doc: QuotationDocumentDto) => {
  ...
  const handleRemove = (doc: QuotationDocumentDto) => {
    removeDocument.mutate(
      { quotationId, documentId: doc.documentId },
      {
        onSuccess: () => {
  ...
  const handleGenerate = () => {
    generate.mutate(
      { quotationId, body: { documentType: 'Summary' } },
      {
        onSuccess: doc => {
...
export default QuotationDocumentsDialog;
