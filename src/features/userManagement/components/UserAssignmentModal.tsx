...
const UserAssignmentModal = ({
  ...
  const [selected, setSelected] = useState<Set<string>>(() => new Set(currentUsers.map(u => u.id)));
  ...
  const selectionLabel = t(
    selected.size === 1 ? 'selectionCount.users' : 'selectionCount.users_other',
    { count: selected.size },
  );
  ...
  return (
    <Modal
      isOpen={isOpen}
      onClose={onClose}
      title={title ?? t('dialogs.assignUsers.title')}
      size="md"
    >
      <div className="flex flex-col gap-4 p-6">
        ...
        <div className="max-h-80 overflow-y-auto border border-gray-100 rounded-lg divide-y divide-gray-50">
          {isLoading ? (
            ...
            <div className="text-center py-8 text-gray-400 text-sm">{t('empty.noUsersFound')}</div>
          ) : (
            filtered.map(u => (
              <label
                key={u.id}
