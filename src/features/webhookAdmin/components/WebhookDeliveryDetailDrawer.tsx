      ...
      {isLoading ? (
        ...
        <div className="flex flex-col gap-5">
          ...
          <div className="grid grid-cols-2 gap-4">
            <LabelValue label={t('columns.id')} value={<code className="text-xs bg-gray-100 px-1.5 py-0.5 rounded">{data.id}</code>} />
            <LabelValue label={t('columns.subscriptionId')} value={<code className="text-xs bg-gray-100 px-1.5 py-0.5 rounded">{data.subscriptionId}</code>} />
            ...
            <LabelValue label={t('columns.lastStatusCode')} value={data.lastStatusCode != null ? String(data.lastStatusCode) : null} />
            <LabelValue label={t('columns.created')} value={new Date(data.createdAt).toLocaleString()} />
            {data.deliveredAt && (
              <LabelValue label={t('columns.deliveredAt')} value={new Date(data.deliveredAt).toLocaleString()} />
            )}
          </div>

          {data.lastError && (
            <div>
              <p className="text-xs font-medium text-gray-500 uppercase tracking-wide mb-1">{t('columns.lastError')}</p>
              <p className="text-sm text-danger bg-danger/5 rounded-lg px-3 py-2">{data.lastError}</p>
            </div>
          )}
          ...
          <div>
            <div className="flex items-center justify-between mb-1">
              <p className="text-xs font-medium text-gray-500 uppercase tracking-wide">{t('drawer.payload')}</p>
              <button
                type="button"
                onClick={handleCopy}
