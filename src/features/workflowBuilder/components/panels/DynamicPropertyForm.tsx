...
import { makeDynamicPropertyFormSchema, type DynamicPropertyFormValues } from '../../schemas';
...
export function DynamicPropertyForm({ nodeId }: Props) {
  ...
  const {
    ...
    reset,
    formState: { errors },
  } = useForm<DynamicPropertyFormValues>({
    resolver: zodResolver(makeDynamicPropertyFormSchema(t)),
    defaultValues: {
      name: data?.name ?? '',
  ...
  return (
    <form onChange={handleSubmit(onSubmit)} className="flex flex-col gap-4">
      <div className="badge badge-neutral gap-1 text-xs">{typeDef?.name || data.type}</div>
      ...
      {typeDef && typeDef.properties.length > 0 && (
        <>
          <div className="divider text-xs">{t('forms.sections.properties')}</div>
          <p className="text-[10px] text-base-content/50">
            {t('forms.hints.dynamicPropertiesCount', { count: typeDef.properties.length })}
          </p>
