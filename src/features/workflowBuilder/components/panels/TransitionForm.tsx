...
import { makeTransitionFormSchema, type TransitionFormValues } from '../../schemas';
...
export function TransitionForm({ edgeId }: TransitionFormProps) {
  ...
  const { register, handleSubmit, reset, watch, setValue } = useForm<TransitionFormValues>({
    resolver: zodResolver(makeTransitionFormSchema()),
    defaultValues: {
      type: initialType,
    ...
    <form onChange={handleSubmit(onSubmit)} className="flex flex-col gap-4">
      ...
      <div className="rounded-lg bg-base-200 p-3 text-xs">
        <div className="flex items-center gap-2">
          <span className="font-medium">{t('forms.connection.from')}</span>
          <span className="badge badge-sm">{sourceName}</span>
        </div>
        <div className="mt-1 flex items-center gap-2">
          <span className="font-medium">{t('forms.connection.to')}</span>
      ...
      <div className="form-control">
        <label className="label">
          <span className="label-text text-xs font-medium">{t('forms.fields.transitionType')}</span>
        </label>
        <select
          value={watchType}
        ...
        >
          <option value="Normal">{t('forms.transitionTypes.normal')}</option>
          <option value="Conditional">{t('forms.transitionTypes.conditional')}</option>
        </select>
      </div>
      ...
      {watchType === 'Conditional' && (
        <div className="form-control">
          <div className="flex items-center justify-between">
            ...
            >
              {isRawMode ? t('forms.buttons.builderMode') : t('forms.buttons.rawMode')}
            </button>
          </div>
          ...
          {isRawMode ? (
            <textarea
              ...
              rows={3}
              placeholder={t('forms.placeholders.decisionBuilderConditionHint')}
  ...
  return (
    <div className="flex gap-2">
      <input
        ...
        placeholder={t('forms.placeholders.conditionBuilderVariableHint')}
        value={parsed.variable}
        onChange={e => onChange(buildCondition(e.target.value, parsed.operator, parsed.value))}
      />
      <select
        className="select select-bordered select-xs"
        value={parsed.operator}
        onChange={e => onChange(buildCondition(parsed.variable, e.target.value, parsed.value))}
      >
        <option value="==">=</option>
        <option value="!=">!=</option>
        <option value=">">{'>'}</option>
      ...
      <input
        className="input input-bordered input-xs flex-1"
        placeholder={t('forms.placeholders.conditionBuilderValueHint')}
        value={parsed.value}
        onChange={e => onChange(buildCondition(parsed.variable, parsed.operator, e.target.value))}
      />
    </div>
  );
}
