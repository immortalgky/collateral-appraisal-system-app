...
const UpdatePaymentRequest = z
  .object({ paymentAmount: z.number(), paymentDate: z.string().datetime({ offset: false }) })
  .passthrough();
const AppraisalFeeItemDto = z
  ...
  .passthrough();
const AppraisalFeePaymentHistoryDto = z
  .object({
    id: z.string().uuid(),
    appraisalFeeId: z.string().uuid(),
    paymentAmount: z.number(),
    paymentDate: z.string().datetime({ offset: false }),
  })
  .passthrough();
const AppraisalFeeDto = z
  .object({
    ...
    createdOn: z.string().datetime({ offset: true }).nullable(),
    items: z.array(AppraisalFeeItemDto).optional(),
    paymentHistory: z.array(AppraisalFeePaymentHistoryDto).optional(),
  })
  .passthrough();
const GetAppraisalFeesResponse = z.object({ fees: z.array(AppraisalFeeDto) }).passthrough();
const CreateAppraisalFeeRequest = z
  .object({ assignmentId: z.string().uuid(), bankAbsorbAmount: z.number().nullish().default(null) })
  .passthrough();
const CreateAppraisalFeeResponse = z.object({ feeId: z.string().uuid() }).passthrough();
const ApproveFeeItemRequest = z.object({ approvedBy: z.string().uuid() }).passthrough();
const AddFeeItemRequest = z
  .object({ feeCode: z.string(), feeDescription: z.string(), feeAmount: z.number().int() })
  .passthrough();
...
const AssignmentDto = z
  .object({
    ...
    assigneeUserId: z.string().uuid().nullable(),
    assigneeCompanyId: z.string().uuid().nullable(),
    externalAppraiserName: z.string().nullable(),
    externalAppraiserLicense: z.string().nullable(),
    ...
    internalAppraiserId: z.string().uuid().nullable(),
    createdOn: z.string().datetime({ offset: true }).nullable(),
  })
  .passthrough();
...
export const schemas = {
  ...
  CreateAppraisalFeeRequest,
  CreateAppraisalFeeResponse,
